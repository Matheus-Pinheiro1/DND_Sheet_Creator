...
class RogueArcaneTricksterSpellPicker extends ConsumerWidget {
  final CreationState state;
  final List<SpellDto> spells;

  const RogueArcaneTricksterSpellPicker({
    super.key,
    required this.state,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final maxSpellLevel =
        RogueChoiceService.arcaneTricksterMaxSpellLevel(state.level);
    final slotSummary = _slotSummary(
      RogueChoiceService.arcaneTricksterSpellSlots(state.level),
    );
    ...
    final knownSpellIds = <String>{
      ...
      RogueChoiceService.mageHandSpellId,
    };

    final cantripOptions = RogueChoiceService.filterArcaneTricksterCantrips(
      spells: spells,
      knownSpellIds: knownSpellIds.difference(selectedCantrips.toSet()),
    );
    final spellOptions = RogueChoiceService.filterArcaneTricksterPreparedSpells(
      spells: spells,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Mage Hand is added automatically. Choose Wizard cantrips and prepared Wizard spells for your Rogue level. Intelligence is your spellcasting ability.',
          ...
          _RogueArcaneTricksterSpellGroup(
            ...
            onChanged: (choices) => ref
                ...
                .setRogueArcaneTricksterCantrips(choices),
          ),
          const SizedBox(height: 12),
          _RogueArcaneTricksterSpellGroup(
            title:
            ...
            onChanged: (choices) => ref
                .read(creationProvider.notifier)
                .setRogueArcaneTricksterSpells(choices),
          ),
        ],
      ),
    );
...
class _RogueArcaneTricksterSpellGroup extends StatelessWidget {
  final String title;
  ...
  const _RogueArcaneTricksterSpellGroup({
    required this.title,
